...
describe 'AntAlgorithms::UPMS::LocalSearch::LS3' do
  subject(:ls) do
    AntAlgorithms::UPMS::LocalSearch::LS3.new(
  ...
  describe '#neighbor' do
    it 'returns new path' do
      ...
      expect(new_path.schedule).to eq [[0], [1, 2]]
      expect(new_path.path).to eq [[0, 0], [1, 1], [1, 2]]
      expect(new_path.cost).to eq 5
    end

    context 'when target mashine has 0 jobs' do
      let(:path) do
        AntAlgorithms::UPMS::PathBuilder::Path.new(
          path: nil,
          cost: nil,
